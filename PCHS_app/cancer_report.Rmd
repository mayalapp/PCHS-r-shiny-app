...
title: "`r params$screening.type` Report `r Sys.Date()`" 
...
# setting up axes for plots - note: titles of plots based on params$screening.type
 ...
 title.patients = ""
   ...
   }else if(params$screening.type == "Mammogram Screening"){
...
if(params$screening.type != ""){
      p2 = p2 + 
        # line showing where measurements changed from all patients in past 3 years to past 12 months 
        geom_vline(xintercept = as.Date("11/01/2020", format = "%m/%d/%Y"), color = "grey", linetype = "dashed")+ 
        annotate("text",x = as.Date("11/01/2020", format = "%m/%d/%Y"), y = max(params$screening.data$screening_rate) + 5, 
                 label = "Redefine 'All Patients'", color = "grey")
}
